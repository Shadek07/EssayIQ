...
angular.module('conceptvectorApp')
  .controller('analyzewholesubmissionsCtrl', ['$scope', '$http', 'serverURL', '$routeParams', 'AutoComplete', 'recommend', 'AuthService',  function($scope, $http, serverURL, $routeParams, AutoComplete, recommend, AuthService) {
    ...
    $scope.data = null;
    $scope.themes = null;
    $scope.loadingAnalysis = $http.get(serverURL + '/wholesubmissions/' + $routeParams.assignmentID + '/' + AuthService.getUserId().toString(), {withCredentials: true, contentType : "application/json"}).success(function(data) {
        console.log('whole submission data', data);
        $scope.data = data;
      })
          .error(function(data) {
            $scope.analyzeError = true;
            $scope.analyzeSuccess = false;
       });
      ...
      $scope.$watchGroup(['data', 'themes'], function(newValues, oldValues, scope) {
            var themedict = {};
            var themedictrev = {};
            var themecolordict = {};
            var themewise_submissionCount = {};
            if (newValues[1] === null){
                return;
            }
            if(newValues[0] === null){
            ...
            {
                var themeid = $scope.themes[i]['id'];
                themedict[$scope.themes[i]['themeName']] = $scope.themes[i]['id'];
                themedictrev[$scope.themes[i]['id']] = $scope.themes[i]['themeName'];
                themecolordict[$scope.themes[i]['id']] = $scope.themes[i]['color'];
                for (var j = 0; j< $scope.data.length; j++){
                    if( $scope.data[j]['themeids'].includes(themeid)){
                        themewise_submissionCount[themeid] = (themewise_submissionCount[themeid]+1) || 1 ;
                    }
                }
             }
             $scope.labels = [];
             var array  =[];
             $scope.chartcolors = [];
             for (var key in themewise_submissionCount) {
                  array.push(themewise_submissionCount[key]);
                  $scope.labels.push(themedictrev[key]);
                  $scope.chartcolors.push(themecolordict[key]);
              }
              $scope.datachart = [array];
              console.log('datachart', $scope.datachart);
              console.log('labels', $scope.labels);
              console.log('colors', $scope.chartcolors);
      });
      ...
      $scope.$watch('updatecount', function(newValue) { //watching $scope.myResource for changes
            $http.get(serverURL + '/wholesubmissions/' + $routeParams.assignmentID + '/' + AuthService.getUserId().toString(), {withCredentials: true, contentType : "application/json"}).success(function(data) {
                console.log('whole submission data', data);
                $scope.data = data;
              });
              $http.get(serverURL + '/themes_filterbyassignment/' + $routeParams.assignmentID, {withCredentials: true, contentType : "application/json"}).success(function(data) {
